...
window.ChatClient = {
    ...
    Start: function (key, hubUrl, assembly, method) {
        ...
        connection.on("ReceiveMessage", (messageId, userId, username, message) => {
            ...
            DotNet.invokeMethodAsync(assembly, method, key, "ReceiveMessage", messageId, userId, username, message);
        });
    ...
    //
    Register: function (key, userId, userName) {
        console.log("Connection register user " + userName);
        var connection = connections[key];
        if (!connection)
            throw "Connection not found for " + key;
        console.log("Connection located");
        // send message (async)
        return connection.invoke("Register", userId, userName);
    },
    // 
    // function called when Blazor client wishes to send a message via SignalR
    //
    Send: function (key, userId, username, message) {
        ...
        console.log("Connection located");
        // send message (async)
        return connection.invoke("SendMessage", userId, username, message);
    },
    //
